    ...
    if (!currentApiBase) {
      setMessages((current) => [
        ...current,
        {
          id: nextId.current++,
          role: "assistant",
          content: "⚙️ Backend URL not set. Click the settings icon in the chat header and paste your Render URL.",
          streaming: false,
        },
      ])
      ...
      setMessages((current) =>
        current.map((message) =>
          message.id === assistantId
            ? {
                ...message,
                content: "Backend unavailable. Make sure your Render service is running and the URL in ⚙️ settings is correct.",
                streaming: false,
              }
            : message,
        ),
      ...
      <div className="relative z-10 w-full max-w-5xl mx-auto grid gap-12 lg:grid-cols-[1fr_1.5fr] items-center">
        ...
        >
          ...
          <p className="text-muted-foreground text-sm leading-relaxed">
            The frontend is a static export deployed on GitHub Pages. The backend runs purely on CPU using ONNX Runtime for ultra-low memory overhead.
          </p>
          <p className="text-muted-foreground text-sm leading-relaxed">
            ...
            {showSettings && (
              ...
              >
                ...
                <p className="text-xs text-muted-foreground text-center max-w-xs">
                  Paste your Render service URL (e.g. <code className="text-primary">https://your-service.onrender.com</code>)
                </p>
                <input
                  autoFocus
                  value={apiInput}
