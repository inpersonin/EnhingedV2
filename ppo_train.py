...
class PolicyWithValueHead(nn.Module):
    ...
    def _get_last_hidden(self, idx: torch.Tensor) -> torch.Tensor:
        ...
        x = self.base.transformer['drop'](self.base.transformer['wte'](idx) + self.base.transformer['wpe'](positions))
...
@torch.no_grad()
def _get_ref_logprobs(ref_model: HinglishGPT, idx: torch.Tensor) -> torch.Tensor:
    _, seq_len = idx.shape
    if seq_len > ref_model.config.block_size:
        idx = idx[:, -ref_model.config.block_size:]
    positions = torch.arange(idx.size(1), device=idx.device)
    x = ref_model.transformer['drop'](ref_model.transformer['wte'](idx) + ref_model.transformer['wpe'](positions))
...
def compute_gae(rewards: torch.Tensor, values: torch.Tensor, dones: torch.Tensor, gamma: float=0.99, lam: float=0.95) -> tuple[torch.Tensor, torch.Tensor]:
    T = rewards.size(0)
...
def ppo_step(policy: PolicyWithValueHead, optimiser: torch.optim.Optimizer, old_logprobs: torch.Tensor, gen_tokens: torch.Tensor, advantages: torch.Tensor, returns: torch.Tensor, values: torch.Tensor, clip_eps: float=0.2, value_coef: float=0.5, entropy_coef: float=0.01, max_grad_norm: float=1.0) -> dict[str, float]:
    new_logprobs_full = policy.get_logprobs(gen_tokens)
    gen_tokens_idx = gen_tokens.unsqueeze(-1)
    new_lp = new_logprobs_full.gather(-1, gen_tokens_idx).squeeze(-1)
    old_lp = old_logprobs.gather(-1, gen_tokens_idx).squeeze(-1)
    ...
    if has_nan_grad:
        optimiser.zero_grad()
        return {'policy_loss': float('nan'), 'value_loss': float('nan'), 'entropy': float('nan'), 'total_loss': float('nan'), 'mean_ratio': float('nan')}
    nn.utils.clip_grad_norm_(policy.parameters_for_opt(), max_grad_norm)
    optimiser.step()
    return {'policy_loss': policy_loss.item(), 'value_loss': value_loss.item(), 'entropy': entropy.item(), 'total_loss': total_loss.item(), 'mean_ratio': ratio.mean().item()}

@torch.no_grad()
def compute_mean_kl(policy: PolicyWithValueHead, ref_model: HinglishGPT, gen_tokens: torch.Tensor) -> float:
    new_lp = policy.get_logprobs(gen_tokens)
    ref_lp = _get_ref_logprobs(ref_model, gen_tokens)
...
def _save_ppo_checkpoint(policy: PolicyWithValueHead, optimiser: torch.optim.Optimizer, iteration: int, best_reward: float, out_dir: str, tag: str='latest') -> str:
    os.makedirs(out_dir, exist_ok=True)
    path = os.path.join(out_dir, f'ppo_{tag}.pt')
    torch.save({'model_state': policy.state_dict(), 'optimiser_state': optimiser.state_dict(), 'model_config': dataclasses.asdict(policy.config), 'iteration': iteration, 'best_reward': best_reward}, path)
    return path
...
def main() -> None:
    ...
    parser.add_argument('--max_iters', type=int, default=500, help='Total PPO iterations. Start conservative (200-500).')
    parser.add_argument('--batch_size', type=int, default=8, help='Prompts per PPO iteration.')
    parser.add_argument('--max_new_tokens', type=int, default=60, help='Generated tokens per rollout (shorter = faster).')
    parser.add_argument('--lr', type=float, default=1e-05, help="Conservative default. Increase only if reward doesn't rise.")
    parser.add_argument('--kl_coef', type=float, default=0.1, help='KL penalty coefficient. Increase if KL blows up.')
    parser.add_argument('--clip_eps', type=float, default=0.2)
    parser.add_argument('--value_coef', type=float, default=0.5)
    parser.add_argument('--entropy_coef', type=float, default=0.01)
    parser.add_argument('--eval_interval', type=int, default=50, help='Log metrics every N iterations.')
    ...
    for iteration in range(args.max_iters):
        ...
        with torch.no_grad():
            for prompt in batch_prompts:
                ...
                out = generate(base_model, idx, max_new_tokens=args.max_new_tokens, temperature=0.9, top_k=50, top_p=0.95, repetition_penalty=1.1, do_sample=True, eos_token_id=50256)
                gen_part = out
                old_lp = policy.get_logprobs(gen_part)
                _, _, val = policy(gen_part)
        ...
        for i in range(len(gen_tokens_list)):
            ...
            stats = ppo_step(policy, optimiser, old_logprobs=old_lp, gen_tokens=gen, advantages=advantages, returns=returns, values=v, clip_eps=args.clip_eps, value_coef=args.value_coef, entropy_coef=args.entropy_coef)
        ...
        if (iteration + 1) % args.eval_interval == 0 or iteration == 0:
            log_entry = {'iter': iteration + 1, 'mean_reward': mean_reward, 'mean_kl': mean_kl, 'mean_shaped_reward': mean_shaped, **total_stats}
            with open(log_path, 'a', encoding='utf-8') as lf:
                lf.write(json.dumps(log_entry) + '\n')
            print(f"iter {iteration + 1:4d} | reward {mean_reward:.4f} | kl {mean_kl:.4f} | shaped {mean_shaped:.4f} | policy_loss {total_stats.get('policy_loss', 0):.4f}")
            if mean_kl > 2.0:
