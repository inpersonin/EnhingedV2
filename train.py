...
def _resolve_dataset(data_dir: str, block_size: int, batch_size: int, device: torch.device):
    bilingual_markers = [os.path.join(data_dir, 'hinglish_train.bin'), os.path.join(data_dir, 'english_train.bin')]
...
class Trainer:
    ...
    def __init__(self, model: HinglishGPT, dataset, out_dir: str, learning_rate: float=3e-05, weight_decay: float=0.1, beta1: float=0.9, beta2: float=0.95, grad_clip: float=1.0, warmup_iters: int=0, lr_decay_iters: int=16000, min_lr: float=3e-06, max_iters: int=16000, batch_size: int=32, gradient_accum_steps: int=1, eval_interval: int=200, eval_iters: int=50, english_ratio: float=0.5, device: Optional[torch.device]=None, dtype: str='bfloat16', seed: int=42) -> None:
        ...
        self.autocast_ctx = torch.amp.autocast(device_type=self.device_type, dtype=torch_dtype) if self.device_type == 'cuda' else nullcontext()
        self.optimiser = model.configure_optimiser(weight_decay=weight_decay, learning_rate=learning_rate, betas=(beta1, beta2), device_type=self.device_type)
        self.iter_num = 0
    ...
    def save_checkpoint(self, tag: str='latest') -> str:
        checkpoint = {'model_state': self.model.state_dict(), 'optimiser_state': self.optimiser.state_dict(), 'model_config': dataclasses.asdict(self.model.config), 'iter_num': self.iter_num, 'best_val': self.best_val, 'best_val_per_language': self.best_val_per_language}
    ...
    def _write_metrics_json(self, losses: dict[str, float]) -> None:
        metrics = {'iter_num': self.iter_num, 'best_val_loss': self.best_val, 'best_val_perplexity': math.exp(self.best_val) if self.best_val != float('inf') else None, 'current_val_loss': losses.get('val'), 'model_parameters': f'{self.model.count_params() / 1000000.0:.2f}M'}
        for language in ('hinglish', 'english'):
            key = f'val_{language}'
            if key in losses:
    ...
    def train(self) -> None:
        ...
        while self.iter_num < self.max_iters:
            ...
            if self.iter_num % self.eval_interval == 0:
                ...
                log_parts = [f'iter {self.iter_num:5d}', f"train loss {losses['train']:.4f}", f"val loss {losses['val']:.4f}", f"val ppl {math.exp(losses['val']):.2f}"]
                ...
                if losses['val'] < self.best_val:
                    ...
                    self.best_val_per_language = {k: v for k, v in losses.items() if k.startswith('val_')}
            ...
            if self.iter_num % 50 == 0:
                ...
                print(f'iter {self.iter_num:5d} | loss {loss.item() * self.gradient_accum:.4f} | {tokens / max(elapsed, 1e-08):,.0f} tok/s')
                tokens = 0
                start_time = time.time()
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--init_from', choices=['scratch', 'gpt2', 'gpt2-medium', 'gpt2-large', 'gpt2-xl'], default='gpt2', help="Default changed from 'scratch' to 'gpt2': fine-tune real pretrained GPT-2 instead of training bilingual competence from random init. See module docstring.")
    parser.add_argument('--block_size', type=int, default=256)
    parser.add_argument('--n_layer', type=int, default=6)
    parser.add_argument('--n_head', type=int, default=6)
    parser.add_argument('--n_embd', type=int, default=384)
    parser.add_argument('--dropout', type=float, default=0.1)
    parser.add_argument('--max_iters', type=int, default=16000, help='Total iterations to run. Resume from best.pt at iter ~8000 -> set this to 16000 for 8000 more steps.')
    parser.add_argument('--batch_size', type=int, default=32)
    parser.add_argument('--grad_accum_steps', type=int, default=1)
    parser.add_argument('--learning_rate', type=float, default=3e-05, help='Fine-tuning default. Use ~6e-4 only with --init_from scratch.')
    parser.add_argument('--weight_decay', type=float, default=0.1)
    parser.add_argument('--warmup_iters', type=int, default=0, help='LR warmup steps. Default 0 because when resuming from a checkpoint the model is already warm.')
    parser.add_argument('--lr_decay_iters', type=int, default=16000, help='Cosine decay runs from warmup end to this iteration. Match max_iters.')
    parser.add_argument('--eval_interval', type=int, default=200)
    parser.add_argument('--eval_iters', type=int, default=50)
    parser.add_argument('--english_ratio', type=float, default=0.5, help='Fraction of every training batch drawn from the English pool (0.0-1.0). Ignored automatically if you only prepared a Hinglish-only (legacy) data dir.')
    ...
    if args.mode == 'train':
        if args.ckpt_path:
            ...
            if 'model_config' not in _peek:
                raise KeyError(f"Checkpoint at '{args.ckpt_path}' has no 'model_config' key. This checkpoint was saved by a different codebase. Inspect it with torch.load() and check its keys.")
            _cfg = GPTConfig(**_peek['model_config'])
            print(f'  Checkpoint model_config: n_layer={_cfg.n_layer}, n_head={_cfg.n_head}, n_embd={_cfg.n_embd}, block_size={_cfg.block_size}, vocab_size={_cfg.vocab_size}')
            del _peek
            model = HinglishGPT(_cfg)
        elif args.init_from == 'scratch':
            model = HinglishGPT(GPTConfig(block_size=args.block_size, vocab_size=50257, n_layer=args.n_layer, n_head=args.n_head, n_embd=args.n_embd, dropout=args.dropout))
        else:
        ...
        model.to(device)
        print(f'Model parameters: {model.count_params() / 1000000.0:.2f}M')
        dataset = _resolve_dataset(data_dir=args.data_dir, block_size=model.config.block_size, batch_size=args.batch_size, device=device)
        trainer = Trainer(model=model, dataset=dataset, out_dir=args.out_dir, learning_rate=args.learning_rate, weight_decay=args.weight_decay, warmup_iters=args.warmup_iters, lr_decay_iters=args.lr_decay_iters, max_iters=args.max_iters, batch_size=args.batch_size, gradient_accum_steps=args.grad_accum_steps, eval_interval=args.eval_interval, eval_iters=args.eval_iters, english_ratio=args.english_ratio, device=device, dtype=args.dtype, seed=args.seed)
        if args.ckpt_path:
