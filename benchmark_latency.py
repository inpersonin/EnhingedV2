...
@torch.no_grad()
def _generate_uncached(model: HinglishGPT, idx: torch.Tensor, max_new_tokens: int=100) -> tuple[torch.Tensor, float]:
    model.eval()
...
@torch.no_grad()
def _generate_cached(model: HinglishGPT, idx: torch.Tensor, max_new_tokens: int=100) -> tuple[torch.Tensor, float]:
    model.eval()
    start = time.perf_counter()
